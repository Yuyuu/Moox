...
public class XmlParser implements ParserInterface{
	...
	private ArrayList<Tag> parseSinceRoot(List<?> l) {
		...
		for (Iterator<?> i = l.iterator();i.hasNext();){
			...
			if (!courant.getTextNormalize().equals(""))t.setValue(courant.getTextNormalize());
			for (Iterator<?> att = courant.getAttributes().iterator(); att.hasNext();){
				notreAttribut = new Attribute();
				org.jdom2.Attribute attributJDOM = (org.jdom2.Attribute)att.next();
	...
	private void afficheListTags(ArrayList<Tag> l, int tabul){
		for (Iterator<Tag> i = l.iterator();i.hasNext();){
			...
			for (Iterator<Attribute> att = tag.getAttributes().iterator();att.hasNext();){

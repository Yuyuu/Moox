...
public class JsonGenerator implements GeneratorInterface {
	 ...
	 * 
	 * @param indent
	 *            Indicates if the generator must indent the parsing result or
		...
		try {
			...
			while (!stack.isEmpty()) {
				...
				for (int i = subNodes.size() - 1; i >= 0; i--) {
					stack.add(0, new Pair<Tag, Integer>(subNodes.get(i),
							level + 1));
				}

				/*
				 * We have to close opened array in the document if the level
				 * decrease
				 */
	...
	private void closeSubTagArray() throws JsonGenerationException, IOException {
		/*
		 * { "example": { "text":null, "attributes":[], "subTags": [ ] --> array
		 * to close } --> tag to close } --> node to close ]
		 */
	 ...
	 */
	private void startNode(String nodeName) throws JsonGenerationException,
			IOException {
		/*
		 * { --> start node "tag": --> node name { --> start tag ...
		 */
	 ...
	 */
	private void writeAttributes(Tag node) throws JsonGenerationException,
			IOException {
		/*
		 * "attributes": [ "atttribute" : "value", ... ]
		 */
		...
		for (Attribute attribute : attributes) {
			...
			this.generator.writeStringField(attribute.getName(),
					attribute.getValue());
	...
	/*
	 * public static void main(String[] args){ String uriIn =
	 * "file:///C:/Users/K%C3%A9vin/Desktop/quiz-moodle-exemple.xml"; String
	 * uriOut = "file:///C:/Users/K%C3%A9vin/Desktop/result.json";
	 * 
	 * 
	 * try { XmlParser p = new XmlParser(); Tag root = p.parse(new URI(uriIn));
	 * 
	 * String test = new JsonGenerator(true).generate(root); FileWriter writer =
	 * new FileWriter(new File(new URI(uriOut))); writer.write(test);
	 * writer.close(); System.out.println(test); } catch (ParserException e) {
	 * // TODO Auto-generated catch block e.printStackTrace(); } catch
	 * (URISyntaxException e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); } catch (GeneratorException e) { // TODO
